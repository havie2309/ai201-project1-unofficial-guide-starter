...
**Why these choices fit the documents:** Each review is 4–6 sentences long,
typically 200–400 characters. Using 250-character chunks with sentence-boundary
splitting means each chunk captures 2–3 complete sentences — enough context to
be meaningful on its own without merging multiple reviews together. The
50-character overlap ensures that key facts near chunk boundaries appear in at
least one complete chunk. Sentence-boundary splitting (regex on punctuation) is
used rather than hard character cuts so chunks never end mid-word or
mid-sentence. An initial attempt at 400-character fixed-split chunks produced
only 71 chunks and cut words mid-syllable; reducing to 250 characters with
sentence splitting produced 135 clean chunks.
...
```
Professor: Collin Nolte | Department: Physics Review 1: Students describe
the professor as knowledgeable and passionate about physics. His lectures
focus on building a strong conceptual understanding before moving into
calculations.
```
...
This confirms grounding is working correctly — the system does not generate
a plausible-sounding answer from general knowledge when the topic is outside
...
### Chunking Strategy Comparison (+1pt)

Two chunking strategies were tested on the same 3 queries:

- **Strategy A (sentence-boundary, chunk_size=250, overlap=50)** — current approach
- **Strategy B (fixed character, chunk_size=400, overlap=80)** — original approach

| Query | Strategy A top result | Strategy A distance | Strategy B top result | Strategy B distance | Winner |
|-------|-----------------------|--------------------|-----------------------|--------------------|--------|
| "Which professor has helpful office hours?" | prof_andrea_hall_reviews.txt — complete sentence about office hours | 0.659 | prof_andrea_hall_reviews.txt — chunk cut mid-word, partial sentence | 0.701 | Strategy A |
| "What do students say about Collin Nolte?" | prof_collin_nolte_reviews.txt — full description of teaching style | 0.521 | prof_collin_nolte_reviews.txt — relevant but merged with unrelated review | 0.598 | Strategy A |
| "Which professor focuses on real-world applications?" | prof_jonathan_wells_reviews.txt — complete sentence about datasets | 0.881 | prof_shonda_kuiper_reviews.txt — partial sentence mid-review | 0.923 | Strategy A |

**Analysis:** Strategy A (sentence-boundary) outperformed Strategy B (fixed
character) on all 3 queries. Fixed character splitting at 400 characters
regularly merged 2–3 reviews into one chunk, diluting the semantic signal.
Sentence-boundary splitting preserves complete thoughts, producing lower
distance scores and more focused retrieval results.

---

### Metadata Filtering (+1pt)

A professor name dropdown was added to the Gradio UI. When a professor is
selected, the retrieval uses ChromaDB's `where` clause to filter results to
only chunks from that professor's file:

```python
results = collection.query(
    query_embeddings=query_embedding,
    n_results=k,
    where={"source": {"$eq": selected_professor}},
    include=["documents", "metadatas", "distances"]
)
```

This allows users to ask "What are the exams like?" while filtering to only
Prof. Sam Rebelsky's reviews, rather than getting chunks from all professors.
The UI shows "All Professors" as the default (no filter applied).

---

### Conversational Memory (+1pt)

Chat history is maintained across turns using a messages list passed to the
Groq API. Each turn appends the user question and assistant response to the
history, allowing follow-up questions like "What about their grading?" to
reference the professor discussed in the previous turn.

Example multi-turn exchange:

Turn 1 — User: "What do students say about Sam Rebelsky?"
Answer: "Students describe Prof. Rebelsky as supportive and approachable,
with overwhelmingly positive feedback for office hours..."

Turn 2 — User: "What about their exams?"
Answer: "Based on the same reviews, Prof. Rebelsky's exams emphasize
understanding over memorization..."

The system correctly resolved "their" in Turn 2 as referring to Sam Rebelsky
from Turn 1, demonstrating working conversational memory.

---

### Hybrid Search (+2pts)

BM25 keyword search was combined with semantic search using Reciprocal Rank
Fusion (RRF). For each query, both methods rank all chunks independently.
RRF combines the rankings using the formula: score = 1/(k + rank), where
...
| Query | Semantic only top result | Hybrid top result | Better method |
|-------|--------------------------|-------------------|---------------|
| "office hours helpful" | prof_andrea_hall — semantically similar but generic | prof_sam_rebelsky — exact keyword "office hours" match + semantic relevance | Hybrid |
| "Collin Nolte teaching style" | prof_collin_nolte — correct | prof_collin_nolte — correct, same result | Tie |
| "real-world applications datasets" | prof_jonathan_wells — correct | prof_jonathan_wells — correct, higher confidence from keyword "datasets" match | Hybrid |

**Analysis:** Hybrid search outperformed semantic-only on 2 of 3 queries.
For queries containing specific keywords like "office hours" or "datasets,"
BM25 boosted chunks with exact keyword matches that semantic search ranked
lower. For professor-name queries, both methods performed equally since the
name appears verbatim in the chunks.